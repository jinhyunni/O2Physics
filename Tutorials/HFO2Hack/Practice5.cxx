...
struct checkpid{
	...
	using FilteredMyTPCPIDTracks = soa::Filtered<MyTPCPIDTracks>;

	void processTPC(FilteredMyTPCPIDTracks const& tracks)
	{
	...
	using FilteredMyTOFPIDTracks = soa::Filtered<MyTOFPIDTracks>;

	void processTOF(FilteredMyTOFPIDTracks const& tracks)
	{
		for( auto const& track : tracks )
		{
			if( track.tpcNClsCrossedRows() < 70 ) continue;
